...
const FKCScreen = () => {
  ...
  const fkc_data = [
    {id:1, title:"Total Time", title2: "18h 30m"}, 
    {id:1, title:"No of Kids", title2: "20"}, 
  ]
    ...
    <View style={{paddingHorizontal:16}}>
      ...
      <View  style={{display:'flex', flexDirection:'row', alignItems:'center', maxWidth:500, justifyContent:'space-between'}}>
        {
          week1_data.map((item, index) =>
              <ShortButtons key={index} bg_color={"#E0EAF9"} active_color={"#6295E2"} onpress={null} text={item.label}/>
          )
        }
      </View >

        {/* timer green card */}
      <View style={{maxWidth:500, marginTop:30, width:"100%", height:windowHeight/2, borderRadius:20, backgroundColor:"#6295E2", alignItems:'center'}}>
        <CustomTextBold style={{fontSize:25, marginTop:15, color:"#fff"}}>Fetal Kick Timer</CustomTextBold>
      ...
      <View style={{maxWidth:500, marginTop:30, flexDirection:'row', flexWrap:'wrap',  justifyContent:"center", paddingLeft:"2%"}}>
        ...
        other_data.map((item, index) => 
          <TouchableOpacity key={index} style={{borderRadius:20, backgroundColor:"#6295E2", width:windowWidth/1.8, height: windowWidth/2.5, flexDirection:'row', justifyContent:'space-between', paddingTop:20, paddingBottom:10, marginBottom:16 }}>
      ...
      <View style={{maxWidth:500, marginTop:30, flexDirection:'row', flexWrap:'wrap',  justifyContent:"center", paddingLeft:"2%"}}>
        { 
        fkc_data.map((item, index) => 
          <TouchableOpacity key={index} style={{borderRadius:20, backgroundColor:"#6295E2", width:windowWidth/1.8, height: windowWidth/2.5, paddingTop:20, paddingBottom:10, marginBottom:16 }}>
            <View style={{paddingLeft:10, alignItems:'center', alignSelf:'center', justifyContent:'center'}}>
              {/* title text */}
              <View style={{flexDirection:'row', alignItems:'center', alignSelf:'center', justifyContent:'center'}}>
                <CustomTextRegular style={{fontSize:25, color:"#fff", textAlign:'center'}}>{item.title}</CustomTextRegular>
                
              </View>
              <CustomTextBold style={{fontSize:25, color:"#fff", textAlign:'center'}}>{item.title2}</CustomTextBold>
            </View>
